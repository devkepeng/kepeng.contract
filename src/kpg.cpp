...
void token::create( const eosio::name&   issuer,
...
void token::issue( const eosio::name& to, const eosio::asset& quantity, const string& memo )
{
...
void token::retire( const eosio::asset& quantity, const string& memo )
{
    auto sym = quantity.symbol;
    eosio::check( sym.is_valid(), "invalid symbol name" );
    eosio::check( memo.size() <= 256, "memo has more than 256 bytes" );
...
void token::transfer( const eosio::name&    from,
                      const eosio::name&    to,
...
void token::sub_balance( const eosio::name& owner, const eosio::asset& value ) {
...
void token::add_balance( const eosio::name& owner, const eosio::asset& value, const eosio::name& ram_payer )
{
...
void token::open( const eosio::name& owner, const eosio::symbol& symbol, const eosio::name& ram_payer )
{
...
void token::close( const eosio::name& owner, const eosio::symbol& symbol )
{
   require_auth( owner );
   accounts acnts( get_self(), owner.value );
